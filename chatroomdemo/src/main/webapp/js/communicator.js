...
function initConnect(onGetMessage){//初始化连接的方法，通信前需调用该方法建立连接
	...
	websocket.onclose = function (e) {//连接关闭的回调方法
	    alert("服务器已断开连接");
	}
...
function getContract(message) //根据服务器消息解析为最终定约的方法，该方法返回一个定约。定约中包含的字段及意义：type、suit、rank字段与前端接口定义相同，banker字段表示庄家位置字母
{
	...
	contract.banker=gson["playerPosition"]["firstLetter"];//这里改为firstLetter？
    console.log(gson["trump"]["suit"]["shortName"]+gson["value"]+gson["playerPosition"]["firstLetter"]);//test
	return contract;
}

function getCard(message)//根据服务器消息解析为玩家所出牌的方法,该方法返回一张牌
{
	console.log("获取牌1"+message.data);
	cardGson=message.data;
	console.log("获取牌2"+cardGson);
	var cardobj=JSON.parse(cardGson);
	var card=new Object();
	card.suit=cardobj["suit"]["shortName"];
	card.rank=cardobj["rank"]["score"];
	console.log("获取牌3"+card.suit);
	return card;
}
...
{
	console.log(callContract.rank+callContract.suit);//测试叫牌输出格式
