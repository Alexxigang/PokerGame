...
public class StateManager {
	...
	private int now=BEFORE_START;//表示当前游戏状态的变量
	...
	public void handleMessage(User user,String message) {
		...
		case BEFORE_START://状态为游戏开始的处理
			...
			break;
		case CALLING:
			if(CallContract(user, message)) {//叫牌结束，设置状态为playing，开始打牌
				now=PLAYING;
			}
			break;
		case PLAYING:
	...
	private void start() {//收到游戏开始消息的响应
		...
		Contract contract=new Contract("N:1C");
		initGame(deckofcards, contract);
	}
	...
	private boolean CallContract(User user,String message){//叫牌过程中对用户消息的响应,若叫牌结束返回true，否则返回false
		...
		System.out.println(callcontract);//测试解析后的叫牌
		...
		if(findendOfCall()) {
			...
			int locOfCallcontract=findBanker(highcallcontract);//找到最早与定约花色相同的叫牌索引
			//根据索引找到对应的定约，然后将对应的叫牌的玩家位置设置给定约的玩家位置
			highcallcontract.setPlayerPosition(callcontractList.get(locOfCallcontract).getPlayerPosition());
			room.sendToAll("call end");//通知玩家叫牌结束
			room.sendToAll(highcallcontract);
			System.out.println(highcallcontract);//测试定约
			//将第一个出牌玩家位置发给所有玩家
			room.sendToAll(BridgeHelper.getNextPlayerPosition(highcallcontract.getPlayerPosition()));
			...
			bridgegame.setContract(highcallcontract);//这里加一下
			return true;
		}
		PlayerPosition position=BridgeHelper.getNextPlayerPosition(room.findPosition(user.getUserId()));//获取下一个玩家的位置
		room.sendToAll(position);
	...
	private boolean findendOfCall() {//内部检测叫牌是否结束的方法
		int flag=0;
		int size=callcontractList.size();
		if(size>=3) {
	...
	private boolean play(User user,String message){//处理玩家出牌消息的方法，若出完则返回true，否则返回false
		...
		Card card=GameParser.getCard(message);//将card的json字符串类型解析为card类
		//将玩家打出的牌发给所有玩家，这里把card类解析成json格式
		room.sendToAll(card);
		//这里不仅判断墩数是不是为0，还要判断当前墩的牌是不是为空，防止在第一墩的四张牌打出去的时候都进入该语句块多次向玩家发出明牌
		if(bridgegame.getTricks().size()==0&&bridgegame.getCurrentTrick().getDeck().getCount()==0) {// getTricks():获取已经完成的所有的墩（时间顺序）。
			System.out.println("刚出第一张牌");
		...
		System.out.println("墩数:"+bridgegame.getTricks().size());
		...
		if(bridgegame.getCardsRemaining()==0) {//获取牌桌上还未打出的牌的数量，如果为0，则打完，结束
			System.out.println("桌上的牌已打完");
			return true;
		}
	...
	public List<CallContract> getCallcontractList() {
		// TODO Auto-generated method stub
		return callcontractList;
	}

	private int findBanker(CallContract hightCallcontract) {//找到最早与定约花色相同的玩家的定约的索引
		Suit suit=hightCallcontract.getTrump().getSuit();
		int order=hightCallcontract.getPlayerPosition().getOrder();
		int loc=-1;
		for(int i=0;i<callcontractList.size()-1;i++) {
			//找出最后加进去的实质性叫品的位置,这里判断花色是否相同以及是否两个玩家为同伴或者为同一玩家
			if(callcontractList.get(i).getTrump().getSuit().equals(suit)&&(callcontractList.get(i).getPlayerPosition().getOrder()==order||callcontractList.get(i).getPlayerPosition().getOrder()==(order+2)%4)) {
				loc=i;
				break;
			}
		}
